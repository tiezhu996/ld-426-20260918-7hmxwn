<template>
  <div class="min-h-screen">
    <aside class="fixed left-0 top-0 z-20 hidden h-full w-60 overflow-y-auto border-r border-ink/10 bg-paper/90 p-6 backdrop-blur md:block">
      ...
      <nav class="mt-10 grid gap-2">
        <RouterLink
          ...
          :key="item.path"
          :to="item.path"
          class="px-3 py-2 text-sm font-semibold text-ink/70 hover:bg-clay hover:text-paper"
    ...
    <header class="sticky top-0 z-10 flex gap-2 overflow-x-auto border-b border-ink/10 bg-paper/95 p-3 md:hidden">
      <RouterLink
        ...
        :key="item.path"
        :to="item.path"
        class="shrink-0 px-3 py-2 text-sm text-ink"
        active-class="bg-ink text-paper"
      >
        {{ item.label }}
      </RouterLink>
